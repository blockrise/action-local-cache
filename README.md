  ...
  my_job:
    ...
    steps:
      ...
      - name: Local cache for API dependencies
        id: api-cache
        uses: blockrise/action-local-cache@1.0.0
        with:
...
For example, when running the usage example above inside a workflow for the `blockrise/product` repo, this empty folder will be created:

```shell
~/runner/_work/_tool/blockrise/product/api-dependencies-v1/
```
